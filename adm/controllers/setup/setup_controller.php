...
class Setup_Controller extends CFX_Controller {
	...
	{
		$this->load->library('Board_Uri', 'uri'); // 게시판판 UR판 라이브러리
	...
	public function lists()
	{
		...
		$this->load->model('setup'); // 모델 로드
		
		$result = array();
		$result = $this->setup_model->get_lists();	// 메소드 호출
	...
	public function view()
	{
		...
		$result = array();
		$result = $this->setup_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			// 설정 업데이트 처리
			$this->load->model('setup'); // 모델 로드			
			$result = $this->setup_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_setup')
			{
			...
			$result = array();
			$result = $this->setup_model->get_update();	// 메소드 호출
