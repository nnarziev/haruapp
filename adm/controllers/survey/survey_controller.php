...
class Survey_Controller extends CFX_Controller {
	...
	{
		$this->set_title('추가 설문'); // 타이틀 설정

		$this->load->model('survey'); // 모델 로드
		
		$result = array();
		$result = $this->survey_model->get_lists();	// 메소드 호출
	...
	public function view()
	{
		...
		$result = array();
		$result = $this->survey_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			...
			// 추가 설문 업데이트 처리
			$this->load->model('survey'); // 모델 로드			
			$result = $this->survey_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_survey')
			{
			...
			$result = array();
			$result = $this->survey_model->get_update();	// 메소드 호출
