...
class Severity_Controller extends CFX_Controller {
	...
	{
		$this->set_title('심각도 평가'); // 타이틀 설정

		$this->load->model('severity'); // 모델 로드
		
		$result = array();
		$result = $this->severity_model->get_lists();	// 메소드 호출
	...
	public function view()
	{
		...
		$result = array();
		$result = $this->severity_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			...
			// 심각도 평가 업데이트 처리
			$this->load->model('severity'); // 모델 로드			
			$result = $this->severity_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_severity')
			{
			...
			$result = array();
			$result = $this->severity_model->get_update();	// 메소드 호출
