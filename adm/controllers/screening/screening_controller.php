...
class Screening_Controller extends CFX_Controller {
	...
	{
		$this->load->library('Board_Uri', 'uri'); // 게시판 URI 라이브러리
	...
	{
		$this->set_title('설정'); // 타이틀 설정

		$this->load->model('screening'); // 모델 로드
		
		$result = array();
		$result = $this->screening_model->get_lists();	// 메소드 호출
	...
	{
		$this->set_title('설문 스크리닝'); // 타이틀 설정

		$this->load->model('screening'); // 모델 로드
		
		$result = array();
		$result = $this->screening_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			...
			// 설문 스크리닝 업데이트 처리
			$this->load->model('screening'); // 모델 로드			
			$result = $this->screening_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_screening')
			{
			...
			$result = array();
			$result = $this->screening_model->get_update();	// 메소드 호출
