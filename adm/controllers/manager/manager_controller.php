...
class Manager_Controller extends CFX_Controller {
	...
	{
		$this->set_title('담당자관리'); // 타이틀 설정

		$this->load->model('manager'); // 모델 로드
		
		$result = array();
		$result = $this->manager_model->get_lists();	// 메소드 호출
	...
	public function view()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			// 담당자관리 메모 등록 처리
			$this->load->model('manager'); // 모델 로드			
			$result = $this->manager_model->post_memo_regist();	// 메소드 호출
			...
			$this->set_title('관리자관리'); // 타이틀 설정

			$this->load->model('manager'); // 모델 로드
			
			$result = array();
			$result = $this->manager_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			// 담당자관리 업데이트 처리
			$this->load->model('manager'); // 모델 로드			
			$result = $this->manager_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_manager')
			{
				if ($result['code'] == 'redirect')
				{			
					$this->router->redirect_link_href('view', DIRECTORY_SEPARATOR.$result['field']);
			...
			$result = array();
			$result = $this->manager_model->get_update();	// 메소드 호출
	...
	public function set_link_href()
	{
		...
		$link_href['pwconfirm'] = CFX_ADMIN_URL.DIRECTORY_SEPARATOR.'pwconfirm'.DIRECTORY_SEPARATOR.'manager';
