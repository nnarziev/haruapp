...
class Group_Controller extends CFX_Controller {
	...
	{
		$this->load->library('Board_Uri', 'uri'); // 멤버 URI 라이브러리
	...
	{
		$this->set_title('프로젝트'); // 타이틀 설정

		$this->load->model('group'); // 모델 로드
		
		$result = array();
		$result = $this->group_model->get_lists();	// 메소드 호출
	...
	public function view()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			// 그룹 모임명 등록 처리
			$this->load->model('group'); // 모델 로드			
			$result = $this->group_model->post_label_regist();	// 메소드 호출

 			if ($result['menu'] == 'label_regist')
			{
			...
			$result = array();
			$result = $this->group_model->get_view();	// 메소드 호출
	...
	public function update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			// 프로젝트 업데이트 처리
			$this->load->model('group'); // 모델 로드			
			$result = $this->group_model->post_update();	// 메소드 호출

 			if ($result['menu'] == 'update_group')
			{
				if ($result['code'] == 'redirect')
				{			
					$this->router->redirect_link_href('view', DIRECTORY_SEPARATOR.$result['field']);
			...
			$result = array();
			$result = $this->group_model->get_update();	// 메소드 호출
	...
	public function label_update()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST')
		{
			...
			$this->load->model('group'); // 모델 로드
			$result = array();
			$result = $this->group_model->post_label_update();	// 메소드 호출

 			if ($result['menu'] == 'label_update')
			{
	...
	public function label_delete()
	{
		// 라벨 삭제 처리
		$this->load->model('group'); // 모델 로드			
		$result = $this->group_model->get_label_delete();	// 메소드 호출

		if ($result['menu'] == 'label_delete')
		{
			if ($result['code'] == 'redirect')
			{
				if ($result['result'] == 'ok')
	...
	public function set_link_href()
	{
		...
		$link_href['label_update'] = CFX_ADMIN_URL.DIRECTORY_SEPARATOR.$class.DIRECTORY_SEPARATOR.'label_update';
		$link_href['label_delete'] = CFX_ADMIN_URL.DIRECTORY_SEPARATOR.$class.DIRECTORY_SEPARATOR.'label_delete'.DIRECTORY_SEPARATOR.$idx;
