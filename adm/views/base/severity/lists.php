				...
				<section class="container">
					<div class="container_inner">
						<article>
							...
							<table summary="<?php echo $CFX->title(); ?>" class="com_table row_table">
								...
								<thead>
									<tr>
										...
										<th scope="row"><span>상세설명</span></th>
										<th scope="row"><span>설문유형</span></th>
										<th scope="row"><span>수정일</span></th>
									</tr>
								</thead>
								<tbody>
								<?php for ($i=0; $i<count($view_data['list']['severity_list']); $i++) { ?>
									<tr>
										<td><span class="title"><?php echo $view_data['list']['severity_list'][$i]['num'] ?></span></td>
										<td><span class="title"><?php echo $view_data['list']['severity_list'][$i]['severity_group'] ?></span></td>
										<td><span class="title"><?php echo $view_data['list']['severity_list'][$i]['severity_order'] ?></span></td>
										<td class="text_left"><span class="title"><a href="<?php echo $view_data['list']['severity_list'][$i]['view_href']; ?>"><?php echo $view_data['list']['severity_list'][$i]['severity_title']; ?></a></span></td>
										<td class="text_left"><span class="title"><a href="<?php echo $view_data['list']['severity_list'][$i]['view_href']; ?>"><?php echo $view_data['list']['severity_list'][$i]['severity_name']; ?></a></span></td>
										<td><span class="title"><?php echo $view_data['list']['severity_list'][$i]['severity_type'] ?></span></td>
										<td><span class="title"><?php echo $view_data['list']['severity_list'][$i]['modified_datetime'] ?></span></td>
									</tr>
								<?php } // end for ?>
								<?php if (count($view_data['list']['severity_list']) == 0) { ?>
									<tr>
							...
							<?php echo $view_data['list']['severity_pages']; ?>
